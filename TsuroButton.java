...
public class TsuroButton extends Button {
  ...
  /**
   * A method to find the endpoint from the unrotated TsuroButton that will replace the input endpoint after rotating 
   * 90 degrees clockwise
   * @param endPoint the endpoint to be replaced by another endpoint after rotating 
   * @return the int representation of the endpoint that will replace the input endpoint after rotating 
   */
  private int findReplacingEndPoint(int endPoint) {
    
    int[][] endPoints = {{0, 2, 5, 7}, {1, 3, 4, 6}};
    int replacingEndPoint = -1;
    
    for(int i = 0; i < endPoints.length; i++) {
      for(int j = 0; j < endPoints[i].length; j++) {
        if(endPoint == endPoints[i][j]) return replacingEndPoint = endPoints[i][(j+3)%4];
      }
    }
    return replacingEndPoint;
  }
  
  /**
   * A method to find the new connection endpoint of the input endpoint after a 90 degree clockwise rotation
   * @param endPoint an endpoint on this TsuroButton
   * @return the int representation of the endpoint that will be the input endpoint's new connection after rotation
   */ 
  private int findNewConnection(int endPoint) {
    ...
    for(int i = 0; i < endPoints.length; i++) {
      for(int j = 0; j < endPoints[i].length; j++) {
        if(endPoint == endPoints[i][j]) return newConnection = endPoints[i][(j+1)%4];
      }
    }
  ...
  /**
   * A method to rotate this TsuroButton 90 degrees clockwise 
   * Endpoint number representation remain constant while the paths change
   */
  public void rotateButton() {
    ...
    for(int i = 0; i < newConnections.length; i++) {
      int replacingEndPointConnection = connections[findReplacingEndPoint(i)];
      int replacingEndPointNewConnection = findNewConnection(replacingEndPointConnection);
      newConnections[i] = replacingEndPointNewConnection;
    }
  ...
  /**
   * A method to check if this TsuroButton has a stone 
   * @return boolean indicating if this TsuroButton has a stone 
   */
  ...
  /**
   * A method to get the endpoint the stone of this TsuroButton is on
   * @return int endpoint that this TsuroButton is one
   */
  public int getStoneEndPoint() {
    
    int endPoint = -1;
    
    for(int i = 0; i < pieces.length; i++) 
      if(pieces[i] != null) endPoint = i;
